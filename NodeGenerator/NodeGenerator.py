...
# divide the whole circle into 5 parts
d_theta = 2 * np.pi / 5

# start at 90 degrees ~(x=0, y=1)
...
for node, (x_pos, y_pos) in pos.items():
    ax.text(x_pos, y_pos, str(node), color='red', ha='center', va='center')

# save in json
node_link_data = nx.node_link_data(G)
node_link_data['connections'] = {}
for node in G.nodes():
    connections = list(G.neighbors(node))
    node_link_data['connections'][node] = connections
